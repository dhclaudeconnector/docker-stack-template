...
const TS_AUTHKEY = process.env.TS_AUTHKEY || env.TS_AUTHKEY;
...
if (!TS_AUTHKEY) {
  console.error('❌  TS_AUTHKEY is not set in .env');
...
if (!TS_AUTHKEY.startsWith('tskey-')) {
  errors.push('Key should start with "tskey-"');
}

if (TS_AUTHKEY.startsWith('tskey-') && !TS_AUTHKEY.startsWith('tskey-auth-')) {
  warnings.push('Key starts with "tskey-" but not "tskey-auth-" — for auth keys the prefix should be "tskey-auth-"');
}

const parts = TS_AUTHKEY.split('-');
if (parts.length < 3) {
  errors.push('Key format looks incorrect — expected format: tskey-auth-<id>-<secret>');
}

if (TS_AUTHKEY.length < 50) {
  warnings.push(`Key is short (${TS_AUTHKEY.length} chars) — verify it was copied completely`);
}
...
if (!errors.length && !warnings.length) {
  console.log(`✅  Key format looks valid (${TS_AUTHKEY.length} chars)`);
}
...
async function main() {
  ...
  const keyPrefix = TS_AUTHKEY.split('-').slice(0, 3).join('-'); // tskey-auth-<id>
  const found = res.body.keys.find(k => k.id && TS_AUTHKEY.startsWith(keyPrefix));
